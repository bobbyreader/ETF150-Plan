...
- A 股指数估值优先使用指数成分股与 A 股个股实时 PE 明细合并后做等权计算，并剔除负 PE 与 PE > 150 样本；若成分股列表或个股 PE 明细接口不可用，则退回指数等权 PE 代理并通过有效样本数暴露。
- 估值样本不足时不会生成买卖动作：有效 PE 样本低于 30 个，或使用指数代理样本时，信号会变为 `data_insufficient`，建议份数为 0。
...
- 成分股严格等权 PE 优先基于 AkShare `stock_zh_a_spot_em` 与中证指数成分股列表合并；若成分股列表或 PE 明细完全缺失，会退回指数等权 PE 代理并在有效样本数上体现。
...
| 模块 | AkShare 数据口径 | 说明 |
| --- | --- | --- |
| A 股估值 | 乐咕乐股指数 PE/PB + 成分股实时 PE | 当前估值优先使用成分股等权样本；历史分位使用乐咕等权 PE 序列 |
...
- 设计原则：不会静默切换到 mock 数据，避免把演示数据误当真实行情；数据质量不足时只允许观察，不允许生成买卖建议。
